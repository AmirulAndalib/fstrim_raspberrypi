...
# Version: 3.4 (Fix USB ID detection and variable handling)
...
VENDOR_ID=""           # Ensure these are treated globally
PRODUCT_ID=""          # Ensure these are treated globally
VERBOSE=0
LOG_FILE=""
ENABLE_TIMER=0
...
# --- Help Function ---
show_help() {
    echo -e "${BOLD}External SSD TRIM Configuration Tool (v3.4)${RESET}" # Version updated
    echo
    ...
    echo -e "${YELLOW}Persistence is handled primarily via udev rules.${RESET}"
    echo
...
# Checks for required commands and optionally installs packages (No changes needed here)
install_packages() {
    ...
    for cmd in "${missing_cmds[@]}"; do
        ...
        if [[ "$pkg" == "systemd" || "$pkg" == "util-linux" || "$pkg" == "udev" ]]; then
            if ! command -v "$cmd" &> /dev/null; then
                 log "ERROR" "Core command '$cmd' from package '$pkg' is missing. Cannot proceed."
                 exit $E_PKG_MISSING
            fi
        fi
    ...
    if [ -n "$update_cmd" ]; then
        ...
        if $update_cmd > /tmp/trim_package_update.log 2>&1; then
             ...
             log "WARNING" "Package list update failed (exit code $?). Check /tmp/trim_package_update.log."
...
# Sets VENDOR_ID and PRODUCT_ID globals directly if found.
# Returns 0 on success, 1 on failure. Echoes the ID string on success.
get_usb_ids() {
    local dev_path=$1
    local vendor_id=""
    local product_id=""
    ...
    # Clear global vars initially within this function's attempt
    VENDOR_ID=""
    PRODUCT_ID=""
    ...
    # Method 1: udevadm info (Check standard and USB-specific keys)
    ...
    if udev_info=$(udevadm info --query=property --name="$base_dev_name" 2>/dev/null); then
        ...
        # If standard keys not found/valid, try USB-specific keys
        if ! [[ "$vendor_id" =~ ^[0-9a-fA-F]{4}$ && "$product_id" =~ ^[0-9a-fA-F]{4}$ ]]; then
            log "TRACE" "Standard ID_VENDOR_ID/ID_MODEL_ID not found or invalid. Checking ID_USB_VENDOR_ID/ID_USB_MODEL_ID."
            vendor_id=$(echo "$udev_info" | grep -E '^ID_USB_VENDOR_ID=' | head -n1 | cut -d= -f2)
            product_id=$(echo "$udev_info" | grep -E '^ID_USB_MODEL_ID=' | head -n1 | cut -d= -f2)
        fi
        ...
        if [[ "$vendor_id" =~ ^[0-9a-fA-F]{4}$ && "$product_id" =~ ^[0-9a-fA-F]{4}$ ]]; then
            ...
            VENDOR_ID="$vendor_id"   # Set global
            PRODUCT_ID="$product_id" # Set global
            found=1
            echo "$ids"; return 0 # Echo ID and return success
        else
             log "TRACE" "udevadm did not provide valid Vendor/Product IDs (checked std and USB keys). Final attempt Vendor='$vendor_id', Product='$product_id'"
    ...
    # Method 2: sysfs path traversal (Only if udevadm failed)
    if [[ "$found" -eq 0 ]]; then
    ...
    # Method 3: lsusb and lsscsi matching (Only if other methods failed)
     if [[ "$found" -eq 0 ]]; then
        ...
        if scsi_info=$(lsscsi -t | grep "$dev_name" | head -n 1); then
            ...
            if [[ -n "$vendor" || -n "$model" ]]; then
                while IFS= read -r line; do
                    ...
                    local usb_id_match=""
                    usb_id_match=$(echo "$line" | grep -o 'ID [[:xdigit:]]\{4\}:[[:xdigit:]]\{4\}' | awk '{print $2}')
                    if [[ -n "$usb_id_match" ]]; then
                        if ( [[ -n "$vendor" && $(echo "$line" | grep -iq "$vendor") ]] || \
                             [[ -n "$model" && $(echo "$line" | grep -iq "$model") ]] ); then
                            log "DEBUG" "Found potential USB ID match via lsusb/lsscsi: $usb_id_match (Matched on Vendor/Model string: '$vendor' / '$model')"
                            VENDOR_ID=$(echo "$usb_id_match" | cut -d: -f1) # Set global
                            PRODUCT_ID=$(echo "$usb_id_match" | cut -d: -f2) # Set global
                            found=1
                            echo "$usb_id_match"; return 0 # Echo ID and return success
                        fi
                    fi
                done < <(lsusb)
...
# Interactively select a USB block device (No changes needed here)
select_usb_device() {
    ...
    while true; do
        ...
        if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le "${#devices_found[@]}" ]; then
            ...
            # Get the USB IDs - capture echoed output and check status
            # Crucially, get_usb_ids now sets VENDOR_ID and PRODUCT_ID globals directly
            usb_id_str=$(get_usb_ids "$DEVICE")
            get_id_status=$? # Capture return status of get_usb_ids

            if [[ $get_id_status -eq 0 && "$usb_id_str" != "unknown" ]]; then
                # Success: get_usb_ids returned 0 and echoed a valid ID
                SELECTED_USB="$usb_id_str" # Store the echoed ID string
                # VENDOR_ID and PRODUCT_ID are already set globally by get_usb_ids
                log "INFO" "Detected USB ID: $SELECTED_USB (Vendor: $VENDOR_ID, Product: $PRODUCT_ID)"
            else
                 # Failure or manual entry needed
                 SELECTED_USB="unknown" # Mark as unknown initially
                 VENDOR_ID=""           # Clear globals
                 PRODUCT_ID=""
                 echo
                 ...
                 if [[ "$manual_usb_id" =~ ^[0-9a-fA-F]{4}:[0-9a-fA-F]{4}$ ]]; then
                     ...
                     VENDOR_ID=$(echo "$manual_usb_id" | cut -d: -f1)   # Set globals from manual input
                     PRODUCT_ID=$(echo "$manual_usb_id" | cut -d: -f2) # Set globals from manual input
                     log "INFO" "Using manually entered USB ID: $SELECTED_USB (Vendor: $VENDOR_ID, Product: $PRODUCT_ID)"
                 else
                     log "WARNING" "Invalid or no manual USB ID entered. Will create a generic udev rule."
                     # SELECTED_USB, VENDOR_ID, PRODUCT_ID remain unknown/blank
                 fi
            fi
            break
        else
...
# Checks if the selected device firmware supports TRIM commands (No changes needed)
check_trim_support() {
    log "INFO" "Checking TRIM/Unmap support for $DEVICE..."
    DEVICE_TRIM_SUPPORTED=0
    DEVICE_MAX_UNMAP_LBA_COUNT=0
    log "DEBUG" "Running: sg_vpd --page=bl \"$DEVICE\""
    local sg_vpd_bl_output
    sg_vpd_bl_output=$(sg_vpd --page=bl "$DEVICE" 2>&1)
    local sg_vpd_bl_status=$?
    if [ $sg_vpd_bl_status -ne 0 ]; then
        log "WARNING" "sg_vpd --page=bl command failed (Exit code $sg_vpd_bl_status)."
        ...
        if [[ "$count_str" =~ ^[0-9]+$ ]]; then
            ...
            log "WARNING" "Could not parse numeric value for 'Maximum unmap LBA count'."
            DEVICE_MAX_UNMAP_LBA_COUNT=0
        fi
    fi
    ...
    local lbpu_supported=0
    if [ $sg_vpd_lbpv_status -ne 0 ]; then
        log "WARNING" "sg_vpd --page=lbpv command failed (Exit code $sg_vpd_lbpv_status)."
        ...
        if echo "$sg_vpd_lbpv_output" | grep -Eiq "Unmap command supported \(LBPU\)[[:space:]]*:[[:space:]]*1"; then
            lbpu_supported=1
             ...
             log "WARNING" "Could not determine LBPU status from sg_vpd output. Assuming not supported."
             lbpu_supported=0
        fi
    fi
    ...
    if [[ "$DEVICE_MAX_UNMAP_LBA_COUNT" -eq 0 || "$lbpu_supported" -eq 0 || $sg_vpd_bl_status -ne 0 || $sg_vpd_lbpv_status -ne 0 ]]; then
        log "INFO" "Primary TRIM check (sg_vpd) failed/negative. Trying hdparm fallback..."
        log "DEBUG" "Running: hdparm -I \"$DEVICE\""
        local hdparm_output
        hdparm_output=$(hdparm -I "$DEVICE" 2>&1)
        local hdparm_status=$?
        if [ $hdparm_status -ne 0 ]; then
             log "DEBUG" "hdparm -I command failed (Exit code $hdparm_status)."
    ...
    if [[ "$lbpu_supported" -eq 1 && "$DEVICE_MAX_UNMAP_LBA_COUNT" -gt 0 ]] || [[ "$hdparm_supported" -eq 1 ]]; then
        ...
        if [[ "$DEVICE_MAX_UNMAP_LBA_COUNT" -eq 0 && "$hdparm_supported" -eq 1 ]]; then
             log "WARNING" "Using default Max Unmap LBA Count (4194304) for calculation."
             DEVICE_MAX_UNMAP_LBA_COUNT=4194304
        fi
    else
        log "ERROR" "Device $DEVICE does not appear to support TRIM/Unmap/Discard."
        DEVICE_TRIM_SUPPORTED=0
        echo -e "\n${RED}${BOLD}WARNING:${RESET} ${RED}TRIM not supported by device/adapter.${RESET}"
        echo -e "${YELLOW}Configuring TRIM may have no effect or cause issues.${RESET}"
        local continue_anyway_response=""
        read -p "Continue anyway? (y/N): " continue_anyway_response
        declare -g continue_anyway="$continue_anyway_response" # Export to global scope
        if [[ ! "$continue_anyway" =~ ^[Yy]$ ]]; then
            log "INFO" "User aborted due to lack of detected TRIM support."
            exit $E_TRIM_UNSUPPORTED_ABORT
        fi
        log "WARNING" "User chose to continue despite lack of detected TRIM support."
        if [[ "$DEVICE_MAX_UNMAP_LBA_COUNT" -eq 0 ]]; then
             DEVICE_MAX_UNMAP_LBA_COUNT=4194304
             log "WARNING" "Using default Max Unmap LBA Count ($DEVICE_MAX_UNMAP_LBA_COUNT)."
        fi
    fi
}

# Calculates discard_max_bytes based on device capabilities (No changes needed)
calculate_discard_max() {
    ...
    log "DEBUG" "Running: sg_readcap --long \"$DEVICE\""
    ...
    if [[ "$DEVICE_MAX_UNMAP_LBA_COUNT" -gt 0 ]]; then
        ...
        DEVICE_DISCARD_MAX_BYTES=0
        log "WARNING" "Max Unmap LBA count is 0 or unsupported. Setting discard_max_bytes to 0."
    fi
    log "INFO" "Calculated discard_max_bytes: $DEVICE_DISCARD_MAX_BYTES"
}

# Creates persistent udev rule based on available IDs
configure_trim() {
    log "INFO" "Applying TRIM configuration for $DEVICE using udev..."
    local device_name=${DEVICE##*/}
    device_name=${device_name%%[0-9]*}
    DEVICE_NAME="$device_name" # Ensure global DEVICE_NAME is set
    ...
    log "DEBUG" "Checking ID availability for udev rule: SELECTED_USB='${SELECTED_USB}', VENDOR_ID='${VENDOR_ID}', PRODUCT_ID='${PRODUCT_ID}'"

    # Check if VENDOR_ID and PRODUCT_ID are non-empty
...
# Created by setup-trim-final.sh (v3.4) for device matching $DEVICE
...
EOF
        ...
        log "WARNING" "USB Vendor/Product ID unavailable or incomplete (Vendor: '${VENDOR_ID}', Product: '${PRODUCT_ID}'). Creating generic udev rule based on kernel name ($device_name)."
        log "WARNING" "This rule might affect other devices if kernel names change."
...
# Created by setup-trim-final.sh (v3.4) for device $DEVICE
...
EOF
    ...
    if udevadm control --reload-rules; then
        ...
        log "WARNING" "Failed to reload udev rules (udevadm control exit code $?)."
    fi
    ...
    if [ -e "$DEVICE" ]; then
        if udevadm trigger --action=change --name="$DEVICE"; then
             ...
             log "WARNING" "Failed to trigger udev for $DEVICE (udevadm trigger exit code $?)."
        fi
    else
         if udevadm trigger --action=change --subsystem-match=block; then
              log "DEBUG" "udevadm trigger successful for block subsystem."
         else
              log "WARNING" "Failed to trigger udev for block subsystem (udevadm trigger exit code $?)."
         fi
    fi
...
# Configures the systemd fstrim timer (No changes needed)
configure_trim_timer() {
    ...
    if ! systemctl list-unit-files "$service_unit" | grep -q "$service_unit"; then
         log "ERROR" "$service_unit not found. Ensure 'util-linux' is correctly installed."
         return $E_SYSTEMD
    fi
...
EOF
    ...
    if ! systemctl daemon-reload; then
        log "WARNING" "systemctl daemon-reload failed (Exit code $?)."
    fi
    log "INFO" "Enabling $timer_unit..."
    if ! systemctl enable "$timer_unit"; then
         log "WARNING" "Failed to enable $timer_unit (Exit code $?). Check 'systemctl list-unit-files'."
    fi
    log "INFO" "Restarting $timer_unit to apply schedule..."
    if ! systemctl restart "$timer_unit"; then
         log "WARNING" "Failed to restart $timer_unit (Exit code $?). Check 'systemctl status $timer_unit'."
    fi
    sleep 1
    ...
    if [[ "$timer_active" == "active" && "$timer_enabled" == "enabled" ]]; then
        ...
        log "INFO" "Current schedule: $timer_sched (Expected: $TIMER_SCHEDULE)"
         if [[ "$timer_sched" == *"$TIMER_SCHEDULE"* ]]; then
              ...
              log "WARNING" "Timer schedule '$timer_sched' might not match desired '$TIMER_SCHEDULE'."
         fi
    else
        ...
        log "WARNING" "Review systemd status: 'systemctl status $timer_unit'."
    fi
}

# Attempts to run fstrim on mounted filesystems (No changes needed)
test_trim() {
    ...
    local base_device_name="$DEVICE_NAME"
    local mount_points
    mount_points=$(lsblk -rno MOUNTPOINT "/dev/${base_device_name}"* 2>/dev/null | grep -v '^$' | head -n 1)
    if [ -n "$mount_points" ]; then
         ...
         log "INFO" "Cannot perform specific fstrim test."
         return 0
    fi
    ...
    if [ $fstrim_status -eq 0 ]; then
        ...
        if echo "$fstrim_output" | grep -q -i "the discard operation is not supported"; then
            log "ERROR" "fstrim test FAILED on $test_mount_point: Discard operation not supported."
            log "ERROR" "Reasons: Filesystem type/mount options ('discard' needed?), device/rule issue (reboot?)."
            ...
            log "DEBUG" "FS type: '$fs_type'. Mount options: '$mount_opts'."
            if [[ -n "$mount_opts" && ! "$mount_opts" =~ discard ]]; then
                log "INFO" "Try remounting with discard: sudo mount -o remount,discard \"$test_mount_point\""
                log "INFO" "Add 'discard' to /etc/fstab options for persistence."
            fi
        else
            log "ERROR" "fstrim test FAILED on $test_mount_point (Exit code: $fstrim_status)."
            log "ERROR" "Output: $fstrim_output"
            log "ERROR" "Check fsck, mount status, dmesg, ensure udev rules applied (reboot?)."
        fi
        return 1
    fi
}
...
if [[ $? -ne 0 ]]; then
    echo "Invalid arguments." >&2; show_help; exit $E_ARGS
fi
eval set -- "$VALID_ARGS"
while true; do
    case "$1" in
        ...
        -v | --verbose) ((_VERBOSE++)); shift ;;
        -l | --log) _LOG_FILE="$2"; shift 2 ;;
        -h | --help) show_help; exit $E_SUCCESS ;;
        --) shift; break ;;
        *) log "ERROR" "Internal error parsing arguments!"; exit $E_INTERNAL ;;
    esac
done
DEVICE="$_DEVICE"
...
if [ $_SELECT_USB -eq 1 ] && [ -n "$DEVICE" ]; then
    log "ERROR" "Cannot use both --select-usb (-u) and --device (-d)."
    exit $E_ARGS
elif [ $_SELECT_USB -eq 0 ] && [ -z "$DEVICE" ]; then
     log "ERROR" "Must specify --device (-d) or --select-usb (-u)."
     exit $E_ARGS
fi
...
if [ -n "$LOG_FILE" ]; then
    ...
    if ! touch "$LOG_FILE"; then
        echo "ERROR: Failed to create/touch log file '$LOG_FILE'." >&2
        exit $E_LOG_FILE
    fi
    log "INFO" "--- Trim Setup Script v3.4 Started ---"
    log "INFO" "Logging enabled to file: $LOG_FILE"
else
    log "INFO" "--- Trim Setup Script v3.4 Started (Logging to console only) ---"
fi
...
# Clear potentially stale global IDs before selection/validation
VENDOR_ID=""
PRODUCT_ID=""
SELECTED_USB="unknown"

if [ $_SELECT_USB -eq 1 ]; then
    select_usb_device # Sets DEVICE, DEVICE_NAME, SELECTED_USB, VENDOR_ID, PRODUCT_ID globals
else
    ...
    local usb_id_str get_id_status
    usb_id_str=$(get_usb_ids "$DEVICE") # Tries to set VENDOR_ID, PRODUCT_ID globals
    get_id_status=$?

    if [[ $get_id_status -eq 0 && "$usb_id_str" != "unknown" ]]; then
        SELECTED_USB="$usb_id_str"
        # VENDOR_ID and PRODUCT_ID should have been set globally by get_usb_ids
        log "INFO" "Detected USB ID for $DEVICE: $SELECTED_USB (Vendor: $VENDOR_ID, Product: $PRODUCT_ID)"
    else
        log "WARNING" "Could not determine USB ID for $DEVICE. Will create a generic udev rule."
        # SELECTED_USB, VENDOR_ID, PRODUCT_ID remain unknown/blank
    fi
fi
# Log final decision state *after* selection/validation is complete
log "DEBUG" "Proceeding with device: $DEVICE (Name: $DEVICE_NAME), USB ID: $SELECTED_USB (Vendor: $VENDOR_ID, Product: $PRODUCT_ID)"

# --- Core Logic ---
check_trim_support
calculate_discard_max
configure_trim           # Creates udev rule based on VENDOR_ID/PRODUCT_ID check
configure_trim_timer
test_trim

# --- Final Summary ---
echo
echo -e "${BOLD}${GREEN}--- TRIM Configuration Summary (v3.4) ---${RESET}"
echo -e "${GRAY}==============================================${RESET}"
echo -e "Target Device:         ${BOLD}$DEVICE${RESET} (Kernel name: ${DEVICE_NAME})"
# Use quotes around variable expansion for safety
echo -e "USB Vendor:Product ID: ${BOLD}\"${SELECTED_USB}\"${RESET} (Vendor: \"${VENDOR_ID}\", Product: \"${PRODUCT_ID}\")"
...
if [ "$DEVICE_TRIM_SUPPORTED" -eq 1 ]; then
    ...
    if [[ -v continue_anyway && "$continue_anyway" =~ ^[Yy]$ ]]; then
        echo -e "Firmware TRIM Support: ${RED}Not Detected${RESET} - ${YELLOW}Proceeded at user request.${RESET}"
         ...
         echo -e "Firmware TRIM Support: ${RED}Not Detected${RESET}" # Assume aborted if check failed and continue_anyway not 'y'
    fi
fi
echo -e "Calculated discard_max: ${BOLD}${DEVICE_DISCARD_MAX_BYTES}${RESET} bytes"

if [ "$UDEV_RULE_CREATED" -eq 1 ]; then
     # Check VENDOR_ID/PRODUCT_ID again to report rule type correctly
     if [[ -n "$VENDOR_ID" && -n "$PRODUCT_ID" ]]; then
    ...
    timer_sched=$(systemctl show -p Timers fstrim.timer 2>/dev/null | grep OnCalendar | cut -d= -f2- | head -n 1)
     if [[ "$timer_active" == "active" && "$timer_enabled" == "enabled" ]]; then
        echo -e "Periodic TRIM Timer:   ${GREEN}Enabled & Active${RESET} (Schedule: ${timer_sched:-$TIMER_SCHEDULE})"
         ...
         echo -e "Periodic TRIM Timer:   ${RED}Error checking status${RESET} (Configured: $TIMER_SCHEDULE)"
     else
...
echo -e "${YELLOW}A system reboot is ${BOLD}strongly recommended${RESET}${YELLOW} to ensure udev rules are correctly applied.${RESET}"
echo
